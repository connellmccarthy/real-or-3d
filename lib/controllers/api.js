  ...
  challenges: {
    ...
    get: async function() {
      const challenges = await sql.query({
        sql: "SELECT * FROM `challenges` ORDER BY date_publish DESC"
      })
      return challenges
    },
    getByChallengeID: async function(uuid) {
      const challenge = await sql.query({
        sql: "SELECT * FROM `challenges` WHERE uuid=? LIMIT 1",
        values: [uuid]
      })
      return challenge[0]
    },
    getBySeasonID: async function(season_id) {
      const challenges = await sql.query({
        sql: "SELECT * FROM `challenges` WHERE season=? ORDER BY date_publish DESC",
        values: [season_id]
      })
      return challenges
    }
  ...
  users: {
    ...
    get: async function() {
      const users = await sql.query({
        sql: "SELECT * FROM `users` ORDER BY season_alg DESC"
      })
      return users
    }
  },
  votes: {
    ...
    get: async function(challenge_id) {
      const votes = await sql.query({
        sql: "SELECT * FROM `votes` WHERE challenge=?",
        values: [challenge_id]
      })
      return votes
    }
